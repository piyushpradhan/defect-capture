...
function capturePage(cfg) {
    function createHiDPICanvas(cfg) {
        ...
        (_a = canvas
            .getContext("2d")) === null || _a === void 0 ? void 0 : _a.setTransform(cfg.pixelRatio, 0, 0, cfg.pixelRatio, 0, 0);
        return canvas;
    }
...
function openScreenshotPage(canvas, cfg) {
    ...
    function onwriteend() {
        chrome.downloads.download({
            url: "filesystem:chrome-extension://" +
...
function getPixelRatio() {
    var ctx = document.createElement("canvas").getContext("2d"), dpr = window.devicePixelRatio || 1, bsr = ctx.webkitBackingStorePixelRatio || ctx.backingStorePixelRatio || 1;
    return dpr / bsr;
}
...
(function () {
    ...
    startRecordingButton.onclick = function (event) {
        chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
            if (message.action === "openDevTools") {
                chrome.devtools.inspectedWindow.eval("DevToolsAPI.showPanel('console')", function (result, isException) {
                    if (isException) {
                        console.error("Failed to open DevTools:", isException);
                    }
                });
            }
        });
